...
fun String.shouldFolderBeVisible(
    ...
    // When showHidden, allow dot-prefixed folders but still hide .nomedia system folders
    val isDotFolder = filename.startsWith('.')
    if (!showHidden && isDotFolder) {
        return false
    } else if (includedPaths.contains(this)) {
        return true
    }

    val containsNoMedia = folderNoMediaStatuses.getOrElse("$this/$NOMEDIA") { false } ||
        ((!isRPlus() || isExternalStorageManager()) && File(this, NOMEDIA).exists())

    // If showHidden: show dot-prefixed folders, but still hide .nomedia non-dot folders (system folders)
    if (containsNoMedia && !isDotFolder) {
        return false
    }
