...
class ViewPagerActivity : BaseViewerActivity(), ViewPager.OnPageChangeListener, ViewPagerFragment.FragmentListener {
    ...
    fun refreshMenuItems() {
        ...
        runOnUiThread {
            ...
            binding.mediumViewerToolbar.menu.apply {
                ...
                findItem(R.id.menu_rotate)?.isVisible = false
                val showRotate = currentMedium.isImage() && visibleBottomActions and BOTTOM_ACTION_ROTATE == 0
                findItem(R.id.menu_rotate_right)?.isVisible = showRotate
                findItem(R.id.menu_rotate_left)?.isVisible = showRotate
                findItem(R.id.menu_rotate_one_eighty)?.isVisible = showRotate
    ...
    private fun setupOptionsMenu() {
        binding.mediumViewerToolbar.apply {
            ...
            navigationIcon = null
            // Smaller title for long filenames
            post {
                for (i in 0 until childCount) {
                    val child = getChildAt(i)
                    if (child is android.widget.TextView && child.text == title) {
                        child.textSize = 14f
                        child.maxLines = 1
                        child.ellipsize = android.text.TextUtils.TruncateAt.MIDDLE
                        break
                    }
                }
            }
        ...
        // Back navigation handled by system back gesture
    }
