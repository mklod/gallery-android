...
class MediaActivity : SimpleActivity(), MediaOperationsListener {
    ...
    override fun onResume() {
        ...
        // Re-apply dark toolbar styling after updateColors resets it
        binding.mediaMenu.post { applyDarkToolbarStyle() }
    ...
    private fun refreshMenuItems() {
        ...
        binding.mediaMenu.requireToolbar().menu.apply {
            ...
            findItem(R.id.empty_recycle_bin).isVisible = false
            findItem(R.id.empty_disable_recycle_bin).isVisible = false
            findItem(R.id.restore_all_files).isVisible = false
            findItem(R.id.folder_view).isVisible = false
            ...
            findItem(R.id.unset_as_default_folder).isVisible = false
            findItem(R.id.toggle_filename).isVisible = false
        }
        updateCalendarToggleIcon()
    }

    private fun applyDarkToolbarStyle() {
        val black = android.graphics.drawable.ColorDrawable(0xFF000000.toInt())
        binding.mediaMenu.setBackgroundColor(0xFF000000.toInt())
        val toolbarContainer = binding.mediaMenu.findViewById<android.view.View>(org.fossify.commons.R.id.toolbar_container)
        val searchBarContainer = binding.mediaMenu.findViewById<android.view.View>(org.fossify.commons.R.id.search_bar_container)
        toolbarContainer?.background = black
        searchBarContainer?.setPadding(0, 0, 0, 0)
        searchBarContainer?.setBackgroundColor(0xFF000000.toInt())
    }

    // View modes: 0=calendar, 1=wall, 2=list
    private fun getCurrentViewMode(): Int {
        val viewTypePath = if (mShowAll) SHOW_ALL else mPath
        val viewType = config.getFolderViewType(viewTypePath)
        if (viewType == VIEW_TYPE_LIST) return 2
        val groupingPath = if (mShowAll) SHOW_ALL else mPath
        val currentGrouping = config.getFolderGrouping(groupingPath)
        return if (currentGrouping and GROUP_BY_NONE != 0) 1 else 0
    }

    private fun toggleCalendarView() {
        val groupingPath = if (mShowAll) SHOW_ALL else mPath
        val viewTypePath = if (mShowAll) SHOW_ALL else mPath
        when (getCurrentViewMode()) {
            0 -> {
                // Calendar → Wall
                config.saveFolderGrouping(groupingPath, GROUP_BY_NONE)
            }
            1 -> {
                // Wall → List
                config.saveFolderViewType(viewTypePath, VIEW_TYPE_LIST)
            }
            2 -> {
                // List → Calendar
                config.saveFolderViewType(viewTypePath, VIEW_TYPE_GRID)
                config.saveFolderGrouping(groupingPath, GROUP_BY_DATE_TAKEN_DAILY or GROUP_DESCENDING)
            }
        }
    ...
    private fun setupOptionsMenu() {
        ...
        binding.mediaMenu.requireToolbar().setOnMenuItemClickListener { menuItem ->
            when (menuItem.itemId) {
                R.id.toggle_calendar -> toggleCalendarView()
    ...
    private fun updateMenuColors() {
        // Skip default color update, use dark style instead
        applyDarkToolbarStyle()
    }
    ...
    private fun tryLoadGallery() {
        requestMediaPermissions {
            ...
            // Hide search field, show folder name as persistent toolbar title
            binding.mediaFolderTitle.visibility = android.view.View.GONE
            binding.mediaMenu.toggleHideOnScroll(false)
            // Hide search UI, show clean toolbar with title and back arrow
            val searchIcon = binding.mediaMenu.findViewById<android.view.View>(org.fossify.commons.R.id.top_toolbar_search_icon)
            val searchEdit = binding.mediaMenu.findViewById<android.view.View>(org.fossify.commons.R.id.top_toolbar_search)
            val toolbarContainer = binding.mediaMenu.findViewById<android.view.View>(org.fossify.commons.R.id.toolbar_container)
            val searchBarContainer = binding.mediaMenu.findViewById<android.view.View>(org.fossify.commons.R.id.search_bar_container)
            searchIcon?.visibility = android.view.View.GONE
            searchEdit?.visibility = android.view.View.GONE
            toolbarContainer?.setBackgroundColor(0xFF000000.toInt())
            searchBarContainer?.setPadding(0, 0, 0, 0)
            val toolbar = binding.mediaMenu.requireToolbar()
            ...
            // Make title large and bold, add bottom padding
            toolbar.contentInsetStartWithNavigation = 0
            toolbar.setPadding(toolbar.paddingLeft, toolbar.paddingTop, toolbar.paddingRight, 16)
            toolbar.post {
                for (i in 0 until toolbar.childCount) {
                    val child = toolbar.getChildAt(i)
                    if (child is android.widget.TextView && child.text == toolbar.title) {
                        child.typeface = android.graphics.Typeface.create("sans-serif", android.graphics.Typeface.BOLD)
                        child.textSize = 28f
                        child.setTextColor(0xFFFFFFFF.toInt())
                        child.setPadding(child.paddingLeft, child.paddingTop, child.paddingRight, 12)
                        break
                    }
                }
            }
