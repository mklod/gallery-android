...
class DirectoryAdapter(
    ...
    private fun setupView(view: View, directory: Directory, holder: ViewHolder) {
        ...
        bindItem(view).apply {
            ...
            if (isSelected) {
                dirCheck.background?.applyColorFilter(0xFFFFFFFF.toInt())
                dirCheck.applyColorFilter(0xFF000000.toInt())
            }
            ...
            if (isListViewType) {
                ...
                photoCnt.setTextColor(white)
                dirName.setTextColor(white)
                dirLocation.applyColorFilter(white)
                photoCnt.beVisible()
            } else if (folderStyle == FOLDER_STYLE_ROUNDED_CORNERS) {
                val white = 0xFFFFFFFF.toInt()
                photoCnt.setTextColor(white)
                dirName.setTextColor(white)
                dirLocation.applyColorFilter(white)
