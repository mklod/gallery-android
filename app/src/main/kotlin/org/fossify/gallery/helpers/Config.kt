...
class Config(context: Context) : BaseConfig(context) {
    ...
    fun getFolderGrouping(path: String): Int {
        val defaultGrouping = if (path != SHOW_ALL && !path.isEmpty()) {
            GROUP_BY_DATE_TAKEN_DAILY or GROUP_DESCENDING
        } else {
            groupBy
        }
        var groupBy = prefs.getInt(GROUP_FOLDER_PREFIX + path.lowercase(Locale.getDefault()), defaultGrouping)
